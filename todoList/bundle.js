...
var APP_ID = 'Gmm9BQ3VTdI6btgzLo8RWaO3-gzGzoHsz'; //实现数据保存在LeanCloud，使用户与数据相关联，而不是保存在localStorage（本地）。
//采用第一种方式用户登陆后添加和删除数据的时候，会连接数据库执行数据的增加和删除。
//使用LeanStorage，创建avTodos对象，保存相关数据信息，再执行其他逻辑。

var APP_KEY = 'SFAJ0JzAVMoKFVD223YcDHT5';
...
var app = new _vue2.default({
	...
	methods: {
		...
		getTimes: function getTimes() {
			...
			var currentDate = date.getFullYear() + seperator1 + month + seperator1 + nowDate + " " + hours + seperator2 + minutes + seperator2 + seconds;
			return currentDate;
		},
		...
		fetchTodos: function fetchTodos() {
			...
			if (this.currentUser) {
				...
				query.find().then(function (todos) {
					...
					_this4.todoList.id = avAllTodos.id;
				}, function (error) {
					console.error(error);
				});
			} else {
